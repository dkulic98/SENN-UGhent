...
%  fileNameMode = 1; sinus_amplitude = 0.8*8.2729; minAPdur = 0;
 if nargin >= 30, error('Too many input arguments'); end
...
DISPLAY = 0;
EONS = 0;                       % Coupling with EONS (evaluation of non-sinusoidal magnetic fields) solver 
...
PlotTimeV = 1;                  % Bool: if 1, calculate time plots
PlotTimem = 0; 
PlotTimen = 0; 
PlotTimeh = 0; 
PlotTimep = 0; 
...
y0 = LLy0s;
z0 = LLz0s;
Ielec = LLElecInj;                          % injected current amplitude (mA)
...
TimeScale = 2;              % If different from zero: dynamical timescale -> T = 2*ITps*timescale (timescale times biphasic pulseduration)
...
if CCPE
    if exist(fileName) %#ok<EXIST>
        ...
        if (fileNameMode == 1)
            Ve = (-1)^(Iancat-1).*Ielec.*gmultiply(fnameStruct.s.V{LLconf},Waveform');
        elseif (fileNameMode == 2)
%             CurInjArray = fnameStruct.s.CurInjArray';
%             CurInjArray = (-1)^(Iancat-1).*Ielec.*interp1(fnameStruct.s.timeVxt,CurInjArray,(0:dt:Nt*dt),'linear','extrap')';
            Vxt = fnameStruct.s.Vxt; 
            Vxt = interp1(Vxt(:,1),Vxt(:,2),(0:dt:Nt*dt)','linear','extrap')';
            Ve = (-1)^(Iancat-1).*Ielec.*gmultiply(fnameStruct.s.V{LLconf},Vxt) + sinus_amplitude.*gmultiply(fnameStruct.s.V{LLconf},Waveform');
        end
if Pulse
Np = ceil(Tp/dt-1/2);              % Np*dt+dt/2 = Tp
Ve(:,Np:end) = 0;
...
Ve = -(-1)^(Iancat-1).*Ielec.*gmultiply((dxI./2+[0; cumsum(dxI(1:end-1))]),Waveform);
    ...
    if exist(fileName) %#ok<EXIST>   
        ...
        elseif (fileNameMode == 2)
%             CurInjArray = fnameStruct.s.CurInjArray';
%             CurInjArray = (-1)^(Iancat-1).*IIelecs.*interp1(fnameStruct.s.timeVxt,CurInjArray,(0:dt:Nt*dt),'linear','extrap')';
            Vxt = fnameStruct.s.Vxt; 
            Vxt = interp1(Vxt(:,1),Vxt(:,2),(0:dt:Nt*dt)','linear','extrap')';
            Ve = (-1)^(Iancat-1).*IIelecs.*gmultiply(fnameStruct.s.V{LLconf},Vxt) + sinus_amplitude.*gmultiply(fnameStruct.s.V{LLconf},Waveform');
        end
if Pulse
...
AF = Ve(:,it);          %Ext_Cur array should be defined similar to this
end

%External current definition
% CurInj = CurInjArray(:,it); 
...
b = B*V+C*AF+Vc;            % b = B*V+C*AF+Vc+CurInj; CurInj an array of externally injected current (mA)
